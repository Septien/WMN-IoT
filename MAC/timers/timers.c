...
int timeout_set(const double seconds)
{
    ...
    for (i = 0; i < TIMEOUTS; i++)
        if ((__sync_fetch_and_or(&timeout_state[i], 0) & (TIMEOU_USED | TIMEOUT_ARMED | TIMEOUT_PASSED)) == (TIMEOUT_USED | TIMEOUT_ARMED))
        {
            const double secs = timespec_diff(timeout_timer[i], now);
