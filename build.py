...
def main():
    ...
    cmd = "valgrind --tool=massif --stacks=yes --time-unit=B --massif-out-file=valgrind-output/massif-output.txt bin/linux-x86_64/wmnlora"
    os.system(cmd)
    ...
    cmd = "valgrind --tool=massif --stacks=yes --time-unit=B --massif-out-file=valgrind-output/massif-output-riot.txt bin/linux-x86_64/wmnlora --fullpath-after=/home/phantom/CP_Systems/RTOS/RIOT/ --read-var-info=yes /home/phantom/CP_Systems/Implementations/WMNLoRa/bin/native/main_test.elf /dev/ttyACM0"
    os.system(cmd)
